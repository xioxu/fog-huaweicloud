...
[
  ...
  describe service_class do
    before :all do
      vcr_directory = 'spec/fixtures/huaweicloud/volume' if service_class == Fog::Volume::HuaweiCloud
      vcr_directory = 'spec/fixtures/huaweicloud/volume_v1' if service_class == Fog::Volume::HuaweiCloud::V1
      vcr_directory = 'spec/fixtures/huaweicloud/volume_v2' if service_class == Fog::Volume::HuaweiCloud::V2
    ...
    def v2?
      @service.kind_of? Fog::Volume::HuaweiCloud::V2::Real
    end
    ...
    it 'CRUD volumes' do
      VCR.use_cassette('volume_crud') do
        begin
          ...
          volume.must_be_kind_of Fog::Volume::HuaweiCloud::Volume
    ...
    it 'reads volume types' do
      VCR.use_cassette('volume_type_read') do
        ...
        type.must_be_kind_of Fog::Volume::HuaweiCloud::VolumeType
    ...
    it 'can extend volumes' do
      VCR.use_cassette('volume_extend') do
        begin
          ...
          proc { @service.extend_volume(volume.id, volume_size_small) }.must_raise Fog::Volume::HuaweiCloud::NotFound
        end
      end
    end
    ...
    it 'can create and accept volume transfers' do
      VCR.use_cassette('volume_transfer_and_accept') do
        begin
          ...
          transfer.must_be_kind_of Fog::Volume::HuaweiCloud::Transfer
          ...
          other_service = service_class.new(
            ...
            :huaweicloud_api_key      => ENV['OS_PASSWORD_OTHER'] || 'password',
            :huaweicloud_username     => ENV['OS_USERNAME_OTHER'] || 'demo',
            :huaweicloud_domain_name  => ENV['OS_USER_DOMAIN_NAME'] || 'Default',
            :huaweicloud_project_name => ENV['OS_PROJECT_NAME_OTHER'] || 'demo'
          )
          ...
          # proc { other_service.volumes.get(volume.id) }.must_raise Fog::Compute::HuaweiCloud::NotFound
          ...
          proc { other_service.transfers.accept(bogus_uuid, auth_key) }.must_raise Fog::Volume::HuaweiCloud::NotFound
          ...
          # proc { @service.volumes.get(volume.id) }.must_raise Fog::Compute::HuaweiCloud::NotFound
          # @service.volumes.all(@name_param => volume_name).length.must_equal 0
      ...
      if v2?
        VCR.use_cassette('volume_transfer_and_delete') do
          begin
            ...
            proc { other_service.transfers.accept(transfer_id, auth_key) }.must_raise Fog::Volume::HuaweiCloud::NotFound
          ensure
            # cleanup volume
