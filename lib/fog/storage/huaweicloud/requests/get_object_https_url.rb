    ...
    class HuaweiCloud
      class Real
        ...
        def create_temp_url(container, object, expires, method, options = {})
          ...
          raise ArgumentError, "Storage must be instantiated with the :huaweicloud_temp_url_key option" if @huaweicloud_temp_url_key.nil?
          ...
          object_path_escaped   = "#{@path}/#{Fog::HuaweiCloud.escape(container)}/#{Fog::HuaweiCloud.escape(object, "/")}"
          object_path_unescaped = "#{@path}/#{Fog::HuaweiCloud.escape(container)}/#{object}"
          string_to_sign = "#{method}\n#{expires}\n#{object_path_unescaped}"

          hmac = Fog::HMAC.new('sha1', @huaweicloud_temp_url_key.to_s)
