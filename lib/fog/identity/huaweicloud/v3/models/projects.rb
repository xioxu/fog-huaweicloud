...
require 'fog/identity/huaweicloud/v3/models/project'

module Fog
  module Identity
    class HuaweiCloud
      class V3
        class Projects < Fog::HuaweiCloud::Collection
          model Fog::Identity::HuaweiCloud::V3::Project

          def all(options = {})
            if service.huaweicloud_cache_ttl > 0
              cached_project, expires = Fog::Identity::HuaweiCloud::V3::Project.cache[{:token   => service.auth_token,
                                                                                     :options => options}]
            ...
            if service.huaweicloud_cache_ttl > 0
              cache = Fog::Identity::HuaweiCloud::V3::Project.cache
              cache[{:token => service.auth_token, :options => options}] = [project_to_cache,
                                                                            Time.now + service.huaweicloud_cache_ttl]
              Fog::Identity::HuaweiCloud::V3::Project.cache = cache
            end
            project_to_cache
          end
          ...
          def find_by_id(id, options = []) # options can include :subtree_as_ids, :subtree_as_list, :parents_as_ids, :parents_as_list
            ...
            if service.huaweicloud_cache_ttl > 0
              ...
              cache[{:token => service.auth_token, :id => id, :options => options}] = [
                top_project, Time.now + service.huaweicloud_cache_ttl
              ]
              Fog::Identity::HuaweiCloud::V3::Project.cache = cache
            end
            top_project
          ...
          def project_from_hash(project_hash, service)
            Fog::Identity::HuaweiCloud::V3::Project.new(project_hash.merge(:service => service))
          end
        end
      end
    end
