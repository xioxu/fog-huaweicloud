...
describe "Shindo.tests('Fog::Compute[:huaweicloud] | service requests" do
  before do
    @service_format = {
      "id"              => Integer,
      "binary"          => String,
    ...
    @services = Fog::Compute[:huaweicloud].list_services.body
  ...
  describe "success" do
    ...
    it "#disable_service" do
      Fog::Compute[:huaweicloud].disable_service(
        @service['host'], @service['binary']
      ).body["service"]["status"].must_equal "disabled"
    end

    it "#disable_service_log_reason" do
      disabled_service = Fog::Compute[:huaweicloud].disable_service_log_reason(
    ...
    it "#enable_service" do
      Fog::Compute[:huaweicloud].enable_service(
        @service['host'], @service['binary']
